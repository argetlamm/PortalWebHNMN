...
public class DT_rol_opcion {
    ...
    //private static final String SQL_SELECT_ALL_VISTA = "SELECT * FROM V_tbl_Rol_Opcion";
    ...
    public ArrayList<Tbl_rol_opcion> listarRolOpcion(){
    	...
    	try {
    		...
    		while(rsRolOpc.next()) {
    			...
    			tblRolOpc.setId_rol_opc(rsRolOpc.getInt("id_rolOpcion"));
    ...
    /*public V_tbl_Rol_Opcion obtenerRolOpcion(int idRolOpc){
		...
		try {
			PreparedStatement ps = c.prepareStatement("SELECT * FROM v_tbl_rol_opcion WHERE id_rol_opcion=?", 
			...
			if(rsVisRolOpc.next()) {				
				...
				vtrop.setRol(rsVisRolOpc.getString("rol"));
				vtrop.setId_opcion(rsVisRolOpc.getInt("id_opc"));
				vtrop.setOpcion_desc(rsVisRolOpc.getString("opcion_desc"));
				vtrop.setRuta(rsVisRolOpc.getString("ruta"));
			}
		}catch(Exception e) {
    	...
    	try {
    		...
    		while (rsRolOpc.next()) {
    			if (rsRolOpc.getInt(1) == tropc.getId_rol_opcion()) {
    	    		rsRolOpc.updateInt("id_rol", tropc.getId_rol());
    	    		rsRolOpc.updateInt("id_opc", tropc.getId_opc());
    	    		rsRolOpc.updateRow();
    	    		editado = true;
    	    		break;
    			}
