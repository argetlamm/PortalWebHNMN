		...
		try
		{
			...
			for(FileItem item : items)
			{
				if(item.isFormField()) 
				{
	                ...
	                {
	                	if(fieldName.trim().equals("historia"))
	                	{
	                		guid = fieldValue;
	                	}
	                	else
			...
			for(FileItem item : items)
			{
				...
				if(!uploaded.isFormField())
				{
					...
					{
						System.out.println("Servidor: Verifique que la imagen de la historia cumpla con los requisitos");
						response.sendRedirect(request.getContextPath()+ "/CMS/quienesSomos/editHistoria.jsp?msj=3");						
					}
				}
			}
