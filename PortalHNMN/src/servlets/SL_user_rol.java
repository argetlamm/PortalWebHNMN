	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(opcion) {
			...
			case 2:{
				try {
					System.out.println("Estoy entrando al case");
					tur.setId_user_rol(Integer.parseInt(request.getParameter("IdUserRol")));
					System.out.println("Id_user_rol del Servlet: "+tur.getId_user_rol());
					tur.setId_user(Integer.parseInt(request.getParameter("user")));
					System.out.println("Id_user del Servlet: "+tur.getId_user());
					tur.setId_rol(Integer.parseInt(request.getParameter("rol")));
					System.out.println("Id_rol del Servlet: "+tur.getId_rol());
					
					if(dtur.modificarUserRol(tur)) {
						response.sendRedirect(request.getContextPath()+ "/CMS/seguridad/editUserRol.jsp?msj=1");
					}
					else
					{
						response.sendRedirect(request.getContextPath()+ "/CMS/seguridad/editUserRol.jsp?msj=2");
					}
				}catch(Exception e) {
					e.printStackTrace();
