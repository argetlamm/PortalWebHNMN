...
/**
 * Servlet implementation class SL_mision
 */
@WebServlet("/SL_mision")
public class SL_mision extends HttpServlet {
		...
		try
		{
			...
			for(FileItem item : items)
			{
				if(item.isFormField()) 
				{
	                ...
	                {
	                	if(fieldName.trim().equals("mision"))
	                	{
	                		guid = fieldValue;
	                	}
	                	else
			...
			for(FileItem item : items)
			{
				...
				if(!uploaded.isFormField())
				{
					...
					{	
						...
						{
							response.sendRedirect(request.getContextPath()+ "/CMS/quienesSomos/editMision.jsp?msj=2");
						}
					}
					else
					{
						System.out.println("Servidor: Verifique que la imagen de la misión cumpla con los requisitos");
						response.sendRedirect(request.getContextPath()+ "/CMS/quienesSomos/editMision.jsp?msj=3");						
					}
				}
			}
